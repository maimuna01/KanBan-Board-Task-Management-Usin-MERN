...
api.post('/login', async (req, res) => {
    try {
        // Call login function from your auth controller
        const token = await loginUser(req.body.username, req.body.password);
        res.json({ token }); // Assuming you return a token upon successful login
    } catch (error) {
...
api.post('/signup', async (req, res) => {
    try {
        // Call signup function from your auth controller
        const user = await signupUser(req.body.username, req.body.password);
        res.json({ user }); // Assuming you return the newly created user
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
});
